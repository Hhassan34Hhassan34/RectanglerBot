module.exports = async (client) => {
  ...
  setInterval(() => {
    activities[1] = { name: `${client.guilds.cache.size} servers`, type: 'PLAYING' }; // Update server count
    activities[2] = { name: `${client.users.cache.size} users`, type: 'PLAYING' }; // Update user count
  ...
  for (const guild of leftGuilds) {
    ...
    client.logger.info(`Rectangler has left ${guild.guild_name}`);
  }

  client.logger.info('Rectangler is now online');
  client.logger.info(`Rectangler is running on ${client.guilds.cache.size} server(s)`);
};
